 ...
 * 
 * 객체지향 개발 방법에서는
 * 매개변수의 개수, 타입, 순서가 다르면 같은 이름의 method를 중복하여 작성할 수 있다.
 * 
...
public class InputServiceV2 {
	...
	public Integer inputValue(String title, int start, int end) {
		...
		while (true) {
			...
			// this.inputValue(String title) method가 return 해준값이 null아니면 유효성 검사 수행
			if(intNum != null) {
				// 유효성 검사를 통과하지 못하면
				if(intNum < start || intNum > end) {
					// 메시지를 보여주고
					System.out.printf("입력값 범위는 %d ~ %d 입니다.\n", start, end);
					// 다시 돌아가서 inputValue(String title)을 호출해서 입력받아라
					continue;
				}
			}
			// inputValue(String title)이 null을 return 했거나 유효한 범위의 정수를 return했으면
	...
	public Integer inputValue(String title) {
		...
		while (true) {
			...
			// if조건문이 false 경우 실행되는 부분 굳이 else 를 사용하지 않아도 문제 없는 코드
			Integer intNum = null;
			try {
